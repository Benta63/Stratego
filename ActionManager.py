...
class ActionManager():
	...
	def islegal(xstart, ystart, xend, yend, self):
	...
	def ErrorFunction(type, self):
		print("Error: {:s}".format(type))
		sys.exit()
	
	#For initial placing of troops on the board	
	def placeTroop(StrategoBoard, troop, x,y, self):
		#Can only place in rows 0-3
		if x == 4 or x == 5:
			ErrorFunction("ILLEGAL PLACING, RE-EVALUATE YOUR LIFE!")
		else:
			StrategoBoard.MapData[x][y] = troop
	...
	def Attack(Attacker, Defender, self):
		#As this is a boolean, ties need to be evaluated elsewhere
